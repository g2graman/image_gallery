(function () {
	...
	angular.module('image_gallery', ['ngMaterial', 'ngAria', 'ngMdIcons'])
		...
		.controller("layoutController", function ($scope, $mdSidenav, $animate) {
			$('body')
				.css({
					'font-family': 'Roboto'
				});
			$('.dropzone')
				.css('background-color', '#81CDF2', 'important');
			$('.dropzone')
				.css('color', '#ffffff', 'important');
			$('#dismisser')
				.css('background-color', '#FFFFD1', 'important');
			$scope.openLeftMenu = function () {
				$mdSidenav('left')
					.toggle();
			};
		})
		.controller("galleryController", function ($scope, $mdSidenav, $animate, $mdDialog) {
			$scope.checked = 0;
